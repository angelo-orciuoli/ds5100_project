...
##### Methods:
- `__init__(faces: List[string or number], weights: Optional[List[int or float]] = None)`: Initializes the die with given faces (must be strings or numbers) and optional weights (must be integers or floats). If no weights are provided, each face is assigned a default weight of 1.
